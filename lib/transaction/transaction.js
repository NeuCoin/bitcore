...
Transaction.prototype._validateFees = function() {
  if (this.getFee() > this._getUnspentValue()) {
    //return 'Not enough inputs to fit the fee';
  }
...
/**
 * Set the transaction timestamp. The input must be an number. This parameter is useful for
 * PoS altcoins, which encode and sign the timestamp against its timestamp.
 *
 * @param {number} timestamp
 * return Transaction this, for chaining
 */
Transaction.prototype.setTimestamp = function(timestamp) {
...
Transaction.prototype.refreshFee = function() {
  return this.fee(this._estimateFee());
};
...
Transaction._estimateFee = function(size, amountAvailable) {
  var fee = Transaction.FEE_PER_KB * (1 + Math.floor(size / 1000));
  if (amountAvailable > fee) {
    // Safe upper bound for change address script
    size += Transaction.CHANGE_OUTPUT_MAX_SIZE;
  }
  return Transaction.FEE_PER_KB * (1 + Math.floor(size / 1000));
};

Transaction.MAXIMUM_EXTRA_SIZE = 4 + 4 + 9 + 9 + 4;
